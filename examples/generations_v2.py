...
def _load_env_vars(env_path: str = ".env"):
    """Load env vars (e.g., OPENAI_API_KEY) from a .env file using python-dotenv."""
    try:
        load_dotenv(dotenv_path=env_path, override=False)
    except Exception as exc:
        logger.warning(f"Failed to load env vars from {env_path}: {exc}")


def rewrite_transcript_with_llm(transcript: str, model: str = "gpt-4o-mini") -> str:
    """Rewrite transcript with an LLM to normalize numbers/abbreviations and strip special chars."""
    client = OpenAI()
...
@click.option(
    ...
    is_flag=True,
    default=False,
    help="Whether to rewrite the transcript with an OpenAI LLM before generation.",
)
@click.option(
    "--chunk_method",
    default=None,
...
def main(
    ...
    if rewrite_transcript:
        try:
            logger.info("Rewriting transcript with OpenAI LLM...")
            transcript = rewrite_transcript_with_llm(transcript)
            logger.info("Transcript rewrite finished.")
        except Exception as exc:
            logger.warning(f"Transcript rewrite failed ({exc}). Proceeding with original transcript.")
    ...
    if (DEBUG):
        logger.info("Debug mode is on, exiting before generation.")
        logger.info("Generated messages and chunked text:")
